...
//		$name=iconv("UTF-8","GBK",$name); 
    ...
    Globle::$smarty->assign("vip_id",$vip_id);
...
//                Globle::$smarty->display('phone2/'.$messageDO->mould.'.tpl');
                    $target = $_REQUEST['target'];
                    if($target==""){
                        $target = "home";
                    }
                    Globle::$smarty->display('phone2/'.$messageDO->mould.'/'.$target.'.tpl');
                }
            }else{
                header('Content-Type:text/html; charset=UTF-8');
