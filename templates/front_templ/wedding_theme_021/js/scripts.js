...
*/
$(window).load(function(){var nav=$('nav'),nav_a=$('a',nav),nav_td=$('td',nav),content=$('#content'),splash=true,msie=$.browser.msie&&$.browser.version<9
$('.carousel1').uCarousel({show:4,pageStep:4})
$('.carousel2').uCarousel({show:3,pageStep:3})
$('.scroll').uScroll({axis:'y',mousewheel:true,duration:600,easing:'easeInOutSine'})
content.hide()
$('td',nav).click(function(){var a=$('a',this)
if(a.length)
location.hash=a.attr('href')
return false})
content.tabs({empty:'#!/splash',actFu:function(_){if(_.prev)
_.prev.stop().css({zIndex:2}).animate({left:-602},{duration:600,easing:'easeInOutSine',complete:function(){_.prev.hide()}})
if(_.curr)
_.curr.css({left:0,zIndex:1}).show()}})
nav.find('>table').navs({useHash:true,defHash:'#!/splash',items:'tr>td'}).navs(function(n,_){if(n===_.defHash||n==='close')
show_splash()
else
show_subpages()
if(n==='#!/contacts')
nav_td.eq(-2).addClass('last'),googleMap()
else
nav_td.eq(-2).removeClass('last')
if(_.n===0)
nav_td.eq(1).addClass('frst')
else
nav_td.eq(1).removeClass('frst')
content.tabs(n)})
function show_splash(){if(!show_subpages.ready)
return false
show_subpages.ready=false
nav.removeClass('subpages')
setTimeout(function(){nav.removeClass('hideact')},msie?1:600)
content.stop().animate({width:0},{duration:600,easing:'easeInOutSine',complete:function(){content.hide()}})}
function show_subpages(){if(show_subpages.ready)
return false
show_subpages.ready=true
nav.addClass('subpages')
setTimeout(function(){nav.addClass('hideact')},msie?1:600)
content.stop().width(0).show().animate({width:602},{duration:600,easing:'easeInOutSine'})}
