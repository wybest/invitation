...
$messageDO = MyDB::selectInfoDB(2,$link);
...
if($messageDO!=null){
	...
	Globle::$smarty->assign("vip","亲爱的美女");
	
	
	Globle::$smarty->assign("name",2);
	...
	$user = MyDB::selectUserByIdDB(2,$link);
    ...
    if($info){
    	...
    	if($user->is_pc==1||$user->is_pay==0){
    		if($messageDO->pc_mould == ""){
    			...
    			Globle::$smarty->display('pc/'.$tpl.'.tpl');
    		}
    	}else{
    		header('Content-Type:text/html; charset=UTF-8');
